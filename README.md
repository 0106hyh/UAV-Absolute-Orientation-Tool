...
1.  **开启方法**：在 `config.yaml` 中设置 `run_mode: "sim"`。
2.  **功能描述**：
    *   自动生成理想的圆周/直线飞行轨迹。
    *   根据配置注入指定强度的 GPS 噪声和随机坏点。
    *   自动运行对齐算法，并输出 **Simulation Accuracy Report**。
3.  **核心指标**：
    *   **Scale Error**: 计算比例与设定真值的偏差（通常 < 0.1%）。
    *   **Rotation Error**: 旋转矩阵的角度偏差。
    *   **Robustness**: RANSAC 对注入坏点的剔除成功率。
